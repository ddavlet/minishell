...
int	builtin_cd(const char **argv, t_env *shell_env)
{
	...
	if (!argv[1])
	{
		path = get_variable_value("HOME", shell_env);
		if (!path)
	...
	{
		path = ft_strjoin_free(get_variable_value("HOME", shell_env), &argv[1][1]);
	}
	else
		path = build_path((char *)argv[1]);
	chdir(path);
	cwd = getcwd(NULL, 0);
	append_envp(shell_env, "OLDPWD", get_variable_value("PWD", shell_env));
